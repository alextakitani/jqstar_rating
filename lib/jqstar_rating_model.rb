module JqStarRating # :nodoc:
  class AlreadyRatedError < StandardError
    def to_s
      "Model has already been rated by this user. To allow update of ratings pass :allow_update => true to the jqstar_rateable call."
    end
  end
  ...
  module ClassMethods
    ...
    # * <tt>:allow_update</tt> Set to true if you want users to be able to update their votes (default=true).
    # * <tt>:cache_column</tt> Name of the column for storing the cached rating average.
    #
    # Example:
    #   class Article < ActiveRecord::Base
    #     jqstar_rateable :stars => 10, :cache_column => :custom_column
    #   end
    ...
    def jqstar_rater(options = {})
      has_many :rates, options
    end

    # Maximum value accepted when rating the model. Default is 5.
    #
    # Change it by passing the :stars option to +jqstar_rateable+
    #
    #   jqstar_rateable :stars => 10
    def max_rate_value
      options[:stars]
    end
  end
